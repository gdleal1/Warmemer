...
FreeCamera g_freeCamera(g_CameraTheta, g_CameraPhi, g_CameraDistance); // Instância da classe FreeCamera
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 00579004 - Gabriel Difforeni Leal", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Vetores W e U usados para calcular a movimentacao da camera livre
        ...
        if (g_KeyWPressed)
        {
            g_freeCamera.MoveFoward(speed_cam,delta_t); // Movimenta a camera para frente
        }

        if (g_KeySPressed)
        {
            g_freeCamera.MoveBackward(speed_cam,delta_t); // Movimenta a camera para tras
        }

        if (g_KeyAPressed)
        {
            g_freeCamera.MoveLeft(speed_cam,delta_t); // Movimenta a camera para esquerda
        }

        if (g_KeyDPressed)
        {
            g_freeCamera.MoveRight(speed_cam,delta_t); // Movimenta a camera para direita
        }

        // Computamos a matriz "View" utilizando os parâmetros da câmera para
        // definir o sistema de coordenadas da câmera.  Veja slides 2-14, 184-190 e 236-242 do documento Aula_08_Sistemas_de_Coordenadas.pdf.
        glm::mat4 view = g_freeCamera.GetMatrixCameraView();
